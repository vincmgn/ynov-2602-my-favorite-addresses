...
watch(
  ...
  (open) => {
    if (open && props.address) {
      ...
      description.value = props.address.description || "";
      addressSearch.value = "Emplacement conservé";
    } else if (open) {
      resetForm();
    }
  }
...
const handleSubmit = async () => {
  if (!name.value || (!isEditMode.value && !addressSearch.value)) {
  ...
  try {
    const url = isEditMode.value
      ? `${config.public.apiBase}/addresses/${props.address?.id}`
      : `${config.public.apiBase}/addresses`;
    ...
    successMessage.value = isEditMode.value ? "Adresse modifiée !" : "Adresse ajoutée !";
    setTimeout(() => {
      emit("success");
      resetForm();
    }, 1500);
...
<template>
  <Transition enter-active-class="duration-300 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100"
    ...
    <div v-if="isOpen" class="fixed inset-0 z-[2000] overflow-y-auto" aria-labelledby="modal-title" role="dialog"
      aria-modal="true">
          ...
          <div class="bg-white px-8 pt-8 pb-8">
            <div class="flex items-center justify-between mb-6">
              <h3 id="modal-title" class="text-2xl font-extrabold text-gray-900 tracking-tight">
                {{ isEditMode ? "Modifier l'adresse" : "Nouvelle adresse favorite" }}
              </h3>
              <button type="button"
            ...
            <form class="mt-4 space-y-5" @submit.prevent="handleSubmit">
              <AuthInput id="name" v-model="name" label="Nom de l'endroit" type="text"
                placeholder="ex: Mon restaurant préféré..." required :disabled="isLoading" />

              <AuthInput v-if="!isEditMode" id="address" v-model="addressSearch" label="Adresse physique" type="text"
                placeholder="ex: 10 rue de la Paix, Paris" required :disabled="isLoading" />

              <div class="space-y-1">
                <label for="description" class="block text-sm font-semibold text-gray-700">Description
                  (optionnel)</label>
                <textarea id="description" v-model="description" rows="3" placeholder="Petite note sur cet endroit..."
